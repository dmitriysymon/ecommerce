...
exports.addToCart = async (req, res) => {
    ...
    try {
        ...
        if (existingCartItem.length > 0) {
            ...
            return res.status(200).json({ message: 'Кількість товару оновлено', newQuantity });
        } else {
            ...
            return res.status(201).json({ message: 'Товар додано успішно', cartId: result.insertId });
        }
    } catch (error) {
        console.error('Помилка при оновлені кошика:', error);
        res.status(500).json({ message: 'Помилка при оновлені кошика', error: error.message });
    }
};
...
exports.updateQuantity = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ message: 'Кількість товару оновлено', quantity });
    } catch (error) {
        console.error('Помилка при оновлені кількості товару:', error);
        res.status(500).json({ message: 'Помилка при оновлені кількості товару', error: error.message });
    }
};
...
exports.removeFromCart = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: 'Помилка при видаленні товару з кошика', error: error.message });
    }
};
...
exports.getCartItems = async (req, res) => {
    ...
    try {
        ...
        const [cartItems] = await pool.execute(
            `SELECT cart.cart_id, cart.quantity, cart.image_url, product.name AS product_name, product.price 
             FROM cart 
        ...
        res.status(500).json({ message: 'Помилка при отриманні товарів з кошика', error: error.message });
    }
};
...
exports.getTotalPrice = async (req, res) => {
    ...
    try {
        ...
        console.error('Помилка при отриманні загальної суми товарів в кошику:', error);
        res.status(500).json({ message: 'Помилка при отриманні загальної суми товарів в кошику', error: error.message });
    }
};
